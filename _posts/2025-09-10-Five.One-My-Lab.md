...
### AD/IR Lab
![AD Lab](/assets/Labs/Lab.drawio.png)  
...
To maintain and collect all the logs(Security Event IDs and Sysmon logs), I have a log analytics workspace, on top of which, sits Microsoft Sentinel. It collects logs from On-prem DC and the user device. And it seems that this is it for the environment.  
It really doesn't sound much but I guess when it comes to AD and Azure, setting it up properly takes time. I still do remember, AMA and DCR troubleshooting to get the desried logs and then trying to parse them in a decent format because there is no dedicated connector for Aurora EDR logs. And not to mention the roles and policies. All of the AD environment is inspired from the environments which I have seen in my Clients (There are like 20 clients. I have good security experience, PS - I am searching for a new job :D).  
